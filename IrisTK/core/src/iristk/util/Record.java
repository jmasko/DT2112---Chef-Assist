...
public class Record {
	...
	private HashMap<String, Object> dynamicFields = new HashMap<String,Object>();
	private HashMap<String,Field> classFields = new HashMap<>();
	private HashMap<String,Method> getMethodFields = new HashMap<>();
	private HashMap<String,Method> setMethodFields = new HashMap<>();
	...
	private synchronized Class<?> getFieldClass(String fieldName) {
		if (classFields.containsKey(fieldName)) {
			return classFields.get(fieldName).getType();
		}
		if (setMethodFields.containsKey(fieldName)) {
			return setMethodFields.get(fieldName).getParameterTypes()[0];
		}
		if (getMethodFields.containsKey(fieldName)) {
			return getMethodFields.get(fieldName).getReturnType();
		}
	...
	public synchronized Object get(String field) {
		...
		if (field.contains(":")) {
			...
			Method getMethod = getMethodFields.get(field);
			if (getMethod != null) {
				try {
			...
			Field getField = classFields.get(field);
			if (getField != null) {
				try {
	...
	public synchronized void put(String field, Object value) {
		if (field != null) {
			...
			if (field.contains(":")) {
				...
				Method setMethod = setMethodFields.get(field);
				if (setMethod != null) {
					try {
				...
				Field setField = classFields.get(field);
				if (setField != null) {
					try {
	...
	public synchronized boolean has(String field) {
		...
		if (field.contains(":")) {
			...
			return getMethodFields.containsKey(field) || classFields.containsKey(field) || field.equals("*") || dynamicFields.containsKey(field);
		}
	}
	...
	public synchronized Set<String> getFields() {
		...
		fields.addAll(dynamicFields.keySet());
		fields.addAll(classFields.keySet());
		fields.addAll(getMethodFields.keySet());
		return fields;
	}
	...
	public synchronized Set<String> getPersistentFields() {
		...
		fields.addAll(dynamicFields.keySet());
		fields.addAll(classFields.keySet());
		for (String f : getMethodFields.keySet()) {
			if (setMethodFields.containsKey(f))
				fields.add(f);
		}
		return fields;
	}
	...
	public static class JsonToRecordException extends Exception {
