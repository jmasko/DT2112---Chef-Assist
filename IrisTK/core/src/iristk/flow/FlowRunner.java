...
public class FlowRunner {
	...
	public class FlowThread implements Runnable {
		...
		public boolean callState(Flow subFlow, FlowEventInfo flowEventInfo) { 
		...
		public boolean callState(State subState, FlowEventInfo flowEventInfo) { 
			//debug("Calling state " + subState + " from " + currentState);
			
			State callingState = currentState;
			/*
			while (callingState != triggeringState) {
				callingState.onexit();
				...
				while (true) {
					...
					if (entry) {
						...
						try {
							...
							for (FlowListener listener : listeners) {
								listener.onFlowException(e, new FlowEventInfo(currentState, new Event("state.enter")));
							}
						}
					} else {
						...
						try {
							...
							for (FlowListener listener : listeners) {
								listener.onFlowException(e, new FlowEventInfo(currentState, event));
							}
						}
					}
					if (gotoState != null) {
		...
		public int raiseEvent(Event event, FlowEventInfo flowEventInfo) {
			for (FlowListener listener : listeners) {
