...
public class Situation {
	...
	private final HashMap<String,Body> sensorBodies = new HashMap<>();
	private final HashMap<String,Body> userBodies = new HashMap<>();
	//private final HashMap<String,Agent> speaking = new HashMap<>();
	...
	private synchronized void senseBody(Record bodyevent) {
		...
		for (String bodyId : bodyevent.getRecord("bodies").getFields()) {
			...
			Body oldBody = sensorBodies.get(sensorId + "-" + bodyId);
			
			if (oldBody != null && userBodies.containsKey(oldBody.id)) {
				...
				int expire = eventBody.getInteger("expire", defaultExpire);
				if (expire != 0) {
					Body newBody  = new Body("user-" + bodyIdCount++);
					newBody.sensor = sensorId + "-" + bodyId;
					sensorBodies.put(newBody.sensor, newBody);
					fillBody(newBody, eventBody, sensor);
					userBodies.put(newBody.id, newBody);
				}
			}
		}
	...
	private void fillBody(Body agent, Record body, Sensor sensor) {
		
		int expire = body.getInteger("expire", defaultExpire);
				
		if (expire < 0) {
			agent.expire = expire;
		} else {
			agent.expire = currentTime() + expire;
		}
		
		agent.head = new BodyPart(locate(sensor, (Location) body.get("head:location")));
		if (body.has("handLeft:location")) 
			agent.handLeft = new BodyPart(locate(sensor, (Location) body.get("handLeft:location")));
		else 
			agent.handLeft = null;
		if (body.has("handRight:location"))
			agent.handRight = new BodyPart(locate(sensor, (Location) body.get("handRight:location")));
		else
			agent.handRight = null;
				
		if (body.getRecord("gaze") != null) {
			agent.gaze = new Location(body.getRecord("gaze"));
		} else {
			agent.gaze = null;
		}
		
		if (body.getRecord("head:rotation") != null) {
			Rotation rot = new Rotation(body.getRecord("head:rotation"));
			if (sensor != null) {
				rot = rot.add(new Rotation(-sensor.rotation.x, sensor.rotation.y + 180, sensor.rotation.z));
			}
			agent.head.rotation = rot;
			//agent.attending = Agent.NOBODY;
		} else {
			agent.head.rotation = null;
		...
		*/
		...
		if (body.has("recId"))
			agent.recId = body.getString("recId");
		
		if (body.has("extra"))
			agent.putAllExceptNull(body.getRecord("extra"));
	...
	private synchronized void cleanBodies() {
		for (String agentId : new ArrayList<String>(userBodies.keySet())) {
			Body agent = userBodies.get(agentId);
	...
	private void removeBody(String bodyId) {
		...
		Body agent = userBodies.remove(bodyId);
		if (agent != null) {
			/*
	...
	*/
	...
	public void onEvent(Event event) {
		...
		if (event.getName().equals("sense.body")) {
			...
			user.sensor = event.getString("sensor");
