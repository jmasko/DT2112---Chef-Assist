	...
	*/
	...
	protected IRecognizer createRecognizer(Language language) throws RecognizerException {
		if (!DesktopRecognizer.getLanguages().contains(language.getCode())) {
			throw new RecognizerException("Language " + language  + " not supported by WindowsRecognizer. Supported languages: " + DesktopRecognizer.getLanguages());
		}
	...
	@Override
	public void loadOpenVocabulary(String contextName, Language language) throws RecognizerException {
		getRecognizer(language).loadDictationGrammar(contextName);
		contextLanguage.put(contextName, language);
	}
